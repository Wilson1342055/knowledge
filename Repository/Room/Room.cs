...
namespace Repository.Room
{
    public class Room : IRoom
    {
        ...
        public ExecResult UpdateRoom(RoomModel room)
        {
            string strSql = $"update Room set RoomName='{room.RoomName}',RoomDesc='{room.RoomDesc}', FloorID={room.FloorID} where RoomID ={room.RoomID}";
            int intResult = BestWoDP.DapperHelper.ExceSQL(strSql, BestWoDP.DapperHelper.DBConnection.KnowledgeHelper);
            List<ExceDataResult> listResult = new List<ExceDataResult>();
        ...
        {
            DataTable dt = DapperHelper.QueryGetDT("Room rm with(nolock) join Floor fr with(nolock) on rm.FloorID = fr.FloorID", "rm.RoomID,rm.RoomName '房间名称',fr.FloorName '所在楼层',rm.RoomDesc '房间描述',rm.CreateUser '创建人',rm.CreateTime '创建时间'", $"and rm.EmployeeID={EmployeeID}", BestWoDP.DapperHelper.DBConnection.KnowledgeHelper);
            return new ExecResult { StatusCode = 1, Message = "操作成功", DTData = dt };
        }

        public ExecResult QueryRoomByFloorID(string FloorID)
        {
            DataTable dt = DapperHelper.QueryGetDT("Room rm with(nolock) ", "rm.RoomID,rm.RoomName ", $"and rm.FloorID in ({FloorID})", BestWoDP.DapperHelper.DBConnection.KnowledgeHelper);
            return new ExecResult { StatusCode = 1, Message = "操作成功", DTData = dt };
        }

        public ExecResult QueryRoomByID(int RoomID)
        {
            DataTable dt = DapperHelper.QueryGetDT("Room rm with(nolock) ", "rm.RoomID,rm.RoomName,rm.FloorID,rm.RoomDesc ", $"and rm.RoomID in ({RoomID})", BestWoDP.DapperHelper.DBConnection.KnowledgeHelper);
            return new ExecResult { StatusCode = 1, Message = "操作成功", DTData = dt };
        }
    }
}
